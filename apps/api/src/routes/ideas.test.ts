...
describe('POST /api/ideas/:id/promote', () => {
  ...
  it('transfers the pitch doc to the new feature (feature_id set, idea_id retained)', async () => {
    ...
    const res = await app.request(`/api/ideas/${idea.id}/promote`, json({ horizon: 'next' }));
    expect(res.status).toBe(201);
    const feature = await res.json();
...
describe('PATCH /api/ideas/:id — idea_edited activity', () => {
  it('records idea_edited on the promoted feature feed with changed fields', async () => {
    const idea = await createIdea();
    const feature = await (
      await app.request(`/api/ideas/${idea.id}/promote`, json({ horizon: 'now' }))
    ).json();
...
describe('POST /api/ideas/:id/pitch', () => {
  ...
  it('409s when a pitch already exists', async () => {
    await seedPitchTemplate();
    const idea = await createIdea();
    const first = await app.request(`/api/ideas/${idea.id}/pitch`, json({}));
    expect(first.status).toBe(201);
    const second = await app.request(`/api/ideas/${idea.id}/pitch`, json({}));
  ...
  it('404s on a missing idea', async () => {
    const res = await app.request(
      '/api/ideas/00000000-0000-0000-0000-000000000000/pitch',
      json({}),
    );
    expect(res.status).toBe(404);
  });
